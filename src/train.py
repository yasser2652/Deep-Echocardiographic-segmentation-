...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--learning-rate", dest="lr", type=float, default=None)
...
def main() -> None:
    ...
    for epoch in range(start_epoch, int(config.get("epochs", 100)) + 1):
        ...
        save_checkpoint(run_dir / "last.pt", model, optimizer, scheduler, epoch, config, row)
        if val_mean_dice > best_dice:
            ...
            save_checkpoint(run_dir / "best_dice.pt", model, optimizer, scheduler, epoch, config, row)
